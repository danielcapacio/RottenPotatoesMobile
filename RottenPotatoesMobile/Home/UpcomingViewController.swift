...
class UpcomingViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let url = "\(ApiHelper.baseUrl)\(ApiHelper.upcoming)?api_key=\(ApiHelper.API_KEY)&language=en-US&region=US"
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            let nibName = UINib(nibName: "UpcomingTableViewCell", bundle: nil)
            self.tableView.register(nibName, forCellReuseIdentifier: "upcomingTableViewCell")
            
            self.loadUpcomingMovies(url: url)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if (ApiHelper.MAX_TABLE_ROWS <= upcomingMovies.count) {
            numberOfRows = ApiHelper.MAX_TABLE_ROWS
        } else {
            numberOfRows = upcomingMovies.count
        }
        return numberOfRows
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if upcomingMovies.count > 0 {
            ...
            if let date = releaseDate {
                ...
                cell.label_releaseDate.text = "Release Date: \(date)"
            } else {
                cell.label_year.text = "Error loading year"
                cell.label_releaseDate.text = "Error loading release date"
            }
        }
        return cell
    }
     ...
     */
    func loadUpcomingMovies(url: String) {
            ...
            case .success(let value):
                ...
                for movie in moviesListPage1 {
                    ...
                    for i in 0...(moviesListPage1[0].count - 1) {
                        self.upcomingMovies.append(movie[i])
                    }
                }
