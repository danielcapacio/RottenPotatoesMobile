...
class NowPlayingViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let url = "\(ApiHelper.baseUrl)\(ApiHelper.nowPlaying)?api_key=\(ApiHelper.API_KEY)&language=en-US&region=US"
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            self.loadNowPlayingMovies(url: url)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if (ApiHelper.MAX_TABLE_ROWS <= nowPlayingMovies.count) {
            numberOfRows = ApiHelper.MAX_TABLE_ROWS
        } else {
            numberOfRows = nowPlayingMovies.count
        }
        return numberOfRows
    }
            ...
            case .success(let value):
                ...
                for movie in moviesListPage1 {
                    ...
                    for i in 0...(moviesListPage1[0].count - 1) {
                        self.nowPlayingMovies.append(movie[i])
                    }
                }
